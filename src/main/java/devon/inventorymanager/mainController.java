...
/** This is the onAddPartClicked method.
 * When someone clicks the Add Part button on the screen, this method gets called.
 ...
 * @throws IOException If and input/output error occurs during scene loading.*/
     ...
     * It Handles actions when the modify part button is clicked.
     ...
     * @param actionEvent The event generated by clicking the add product button*/
    ...
    /** This is the Part SearchByName method.
     * This method searches for a part by a name or partial name entered in the text field.
     * @param partialName the name to search for.
     * @return ObservableList of parts with names the match the search criteria. */
...
/** This is the SearchByProductName method.
 * This method searches for a product by a name or partial name entered in the text field.
 * @param partialName the name to search for
 * @return ObservableList of products with names that match the search criteria.*/
    private ObservableList<Product> searchByProductName(String partialName){
        //a list to store the products that match the search
        ObservableList<Product> namedProducts = FXCollections.observableArrayList();

        //get the list of all products from the inventory.
        ObservableList<Product> allProducts = Inventory.getAllProducts();

        //loop through each product in the allProducts list.
        for(Product nProduct: allProducts){
            //check if the name of the current product matches the partial name being searched for
            if(nProduct.getName().contains(partialName)){
                //if the name matches add the product to the list of matching products
    ...
    /** This is the onPartsSearch method.
     * This method is called when the user presses the enter key after entering part name or id
     * in the part search text field.
     * @param actionEvent The event generated when the user presses the enter key*/
    public void onPartsSearch(ActionEvent actionEvent) throws IOException{
        //get the text entered into the part search text field
        String partSearchTF = partSearch.getText();

        //search for parts whose names contain the provided search text
        ObservableList<Part> parts = searchByName(partSearchTF);

        //if no part names match the search criteria try to search by part ID
        if(parts.size() == 0){
            try {
                ...
                //if the input is not a valid part ID ignore the exception
            }

        }
        //update the parts table view with the parts that match the search criteria
        partsTable.setItems(parts);
    }

    /** This is the getPartByID method.
     * This method search for a part by its part ID.
     * @param id the ID of the part being searched for
     * @return The part with the ID that matches the search criteria */
    private Part getPartByID(int id){
        //gets the list of all parts
        ObservableList<Part> allParts = Inventory.getAllParts();

        //loop through all the parts to find the one that matches the search criteria
        for(int i = 0; i < allParts.size(); i++){
            Part idP = allParts.get(i);

            //check if the id of the current part matches the search criteria
            if (idP.getId() == id){
                //return the part with the matching id
                return idP;
            }
        }
        //return null if the id being searched for is not found
        return null;
    }

    /** This is the onProductSearch method.
     * This method is called when the user presses the enter key after entering the product name
     * or id in the product search text field.
     * @param actionEvent the event generated when the user presses the enter key*/
    public void onProductSearch(ActionEvent actionEvent) throws IOException{
        //get the text entered into the product search text field
        ...
        //if no matching product names are found try searching by product id
        if(products.size() == 0){
            try {
        ...
        //update the products table with products that match the search criteria
        productsTable.setItems(products);

    }

    /** This is the getProductByID method.
     * This method searches for a product in the list of all products by its product id.
     * @param id The id of the product being searched for
     * @return the product with the matching id if no match is found returns null*/
    private Product getProductByID(int id){
        //gets the list of all products from the inventory
        ObservableList<Product> allProducts = Inventory.getAllProducts();

        //loop through the list of all products
        for(int i = 0; i < allProducts.size(); i++){
            Product idProd = allProducts.get(i);

            //compare the id of the current product with the product id being searched for
            if(idProd.getId() == id){
                //return if the product id is a match
                return idProd;
            }
        }
